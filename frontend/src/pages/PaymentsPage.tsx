...
import dashboardApi, { PaymentRow, OutstandingItem } from '../api/dashboard';
...
export default function PaymentsPage() {
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const [pays, out] = await Promise.all([
        dashboardApi.listAllPayments(),
        dashboardApi.listOutstanding(),
      ]);
      setPayments(pays);
      setOutstanding(out);
    } catch (err) {
  ...
  const filteredPayments = useMemo(() => {
    ...
    if (filterMonth) {
      list = list.filter((p) => p.paidAt.startsWith(filterMonth));
    }
    return list;
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 py-3 sm:py-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-base sm:text-lg font-bold text-gray-800">결제 · 매출 관리</h1>
            <p className="text-xs text-gray-400 mt-0.5">입금 내역과 미수금을 관리합니다.</p>
          </div>
      ...
      <div className="bg-white border-b border-gray-200 px-4 sm:px-6 flex-shrink-0">
        <div className="flex gap-1">
          <TabButton active={tab === 'payments'} onClick={() => setTab('payments')}>
            💰 입금 내역 <span className="text-gray-400">({payments.length})</span>
          </TabButton>
          <TabButton active={tab === 'outstanding'} onClick={() => setTab('outstanding')}>
            ⚠️ 미수금 <span className="text-gray-400">({outstanding.length})</span>
          </TabButton>
        </div>
      </div>
